#pragma	title("Exception Example  --  Version 1.0  --  (Support.C)")
...
/* This	module contains	routine	used to	load a dialogue	template from	*/
/* the applications resource and place the controls within a specified	*/
/* window, create a window in a	specific location and to perform	*/
...
static INTERNALADDRESSLIST aial[ ] = { { (PFNINTADD)PDSGetTemplate,  "PDSGetTemplate"  },
				       { (PFNINTADD)PDSKeyProc,	     "PDSKeyProc"      },
				       { (PFNINTADD)InitApp,	     "InitApp"	       },
				       { (PFNINTADD)CreateStdWindow, "CreateStdWindow" },
				       { NULL, NULL } };

INTERNALADDRESS	intaddSupport =	 { __FILE__, 0,	aial };
