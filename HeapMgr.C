#pragma	title("Exception Example  --  Version 1.0 -- (HeapMgr.C)")
#pragma	subtitle("  Memory Manager - Interface Definitions")
...
/* This	module contains	the memory routines used by the	custom		*/
/* controls.								*/
...
if ( cbNewBlks % 4096UL	)
   ...
   phbase->cbNewBlks  =	cbNewBlks;
		       ...
		       /* Force	the first to have a used block count	*/
		       /* even though no blocks	have been allocated.	*/
		       /* By doing this, the first block cannot	be	*/
		       /* released.					*/
phbase->cBlksUsed = 1UL;
...
if ( hHeap )
   ...
   memcpy(szStrBuf, "NULL heap handle.", 18);
